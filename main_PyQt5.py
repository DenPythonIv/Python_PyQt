...
class The_choice_of_the_hero(QDialog):
    ...
    def ui_hero(self):
        ...
        for i in range(4):
            ...
            # Доп.Характиристики.
            self.rasa_hero = QLabel()
            self.rasa_hero.setText('Раса : {0}'.format(text[ind_hero]['rasa']))

            self.battle = QLabel()
            self.battle.setText(text[ind_hero]['battle'])

            # Упаковка вертикальном положении.
            self.vbox.addWidget(self.hero)
            self.vbox.addWidget(self.hero_info)
            self.vbox.addWidget(self.rasa_hero)
            self.vbox.addWidget(self.battle)
...
class Window(QtWidgets.QMainWindow, QWidget):
    ...
    def ui_window(self):
        ...
        ee = QPixmap('s.jpg')
        ee = ee.scaled(400, 400)
        self.ui.label_30.setPixmap(ee)
...
if __name__ == '__main__':
    ...
    # Загружаем заставку.
    splash = QSplashScreen(QtGui.QPixmap('glavnay.jpg'))
    splash.showMessage('Загрузка данных .... 0%',
                       QtCore.Qt.AlignCenter | QtCore.Qt.AlignBottom, QtCore.Qt.red)
    splash.setGeometry(230, 210, 800, 600)
    splash.pixmap()
    # ОТображаем заставку
    splash.show()
